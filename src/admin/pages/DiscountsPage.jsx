...
// Định nghĩa các hằng số mapping và phân trang
const STATUS_MAP = {
  active: 1,
  inactive: 2,
};
...
export function DiscountsPage() {
  ...
  const initialFormState = {
    discountTypeId: TYPE_MAP.percentage,
    discountCode: '',
    discountValue: 0,
    discountDescription: '',
  ...
  const statusIdForApi = statusFilter === 'all' ? undefined : STATUS_MAP[statusFilter];
  ...
  const handleDeleteDiscount = async (id) => {
    ...
    const newStatusId = discount.discountStatusId === STATUS_MAP.active
      ? STATUS_MAP.inactive
      : STATUS_MAP.active;

    // 3. Xây dựng payload một cách an toàn
    // Cung cấp giá trị mặc định (ví dụ: 0, '', null) cho các trường [Required] nếu chúng không tồn tại
    // để tránh lỗi và đảm bảo validation ở backend pass.
    const updatePayload = {
    ...
    // THÊM BƯỚC GỠ LỖI QUAN TRỌNG
    // Mở Console (F12) để xem chính xác payload đang được gửi đi
    ...
    try {
      ...
      // 5. Tải lại dữ liệu nếu thành công
      refetch();
    } catch (error) {
      // 6. Xử lý lỗi
  ...
  const getStatusBadge = (statusName) => {
    ...
    return (
      <Badge variant={lowerStatus === 'active' ? 'default' : 'secondary'}>
        {statusName}
      </Badge>
    );
  };
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Select value={String(newDiscount.discountTypeId)} onValueChange={(value) => setNewDiscount({...newDiscount, discountTypeId: Number(value)})}>
                      ...
                      <SelectContent>
                        <SelectItem value={String(TYPE_MAP.percentage)}>Percentage (%)</SelectItem>
                        <SelectItem value={String(TYPE_MAP.fixed)}>Fixed Amount ($)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
            ...
            <Table>
              ...
              <TableBody>{paginatedDiscounts.map((discount) => (
                  <TableRow key={discount.discountId}>
                    ...
                    <TableCell>{discount.discountValue}{discount.discountType?.name === 'Percentage' ? '%' : '$'} off</TableCell>
                    ...
                    <TableCell>{getStatusBadge(discount.discountStatus?.name)}</TableCell>
                    <TableCell>
